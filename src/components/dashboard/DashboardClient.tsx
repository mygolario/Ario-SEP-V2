...
import { TourProvider } from '@/components/guidance/TourProvider';
...
// ... (existing imports - handled by preserving lines if possible, but replace tool replaces chunks)
// I will assume imports are added at the top manually or I replace top section.
// Actually, safely adding imports via a separate call is better or standard replacement.
// Let's do a large replacement of the return statement and imports.
...
export default function DashboardClient({
  ...
  const [activeTab, setActiveTab] = useState(initialData?.journey ? 'journey' : 'classic');
  ...
  useEffect(() => {
    ...
    if (searchParams.get('login') === 'success') {
      ...
      router.replace(newParams.toString() ? `/dashboard?${newParams.toString()}` : '/dashboard');
    }
  ...
  const handleRegenerateOnePagePlan = async () => {
    ...
    try {
      ...
      // If we regenerated and got a journey (future proofing), switch to it.
      // Current regeneration logic is only for one-page-plan, so maybe not needed yet.
    } catch (error) {
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 md:p-12" dir="rtl">
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-6">
            <TabsList className="bg-slate-100 p-1 rounded-xl w-full md:w-auto inline-flex">
              {data.journey && (
                <TabsTrigger
                  value="journey"
                  className="px-8 data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm"
                >
                  نقشه راه اختصاصی (Journey)
                </TabsTrigger>
              )}
              <TabsTrigger
                value="classic"
                className="px-8 data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm"
              >
                بوم مدل و جداول (Classic)
              </TabsTrigger>
            </TabsList>

            <TabsContent value="journey" className="mt-6">
              {data.journey ? <JourneyView journey={data.journey} /> : <div>Plan not found</div>}
            </TabsContent>

            <TabsContent value="classic" className="mt-6 space-y-8">
              <OnboardingChecklist />
                    ...
                    {data.onePagePlan ? (
                      ...
                      <div className="flex flex-col gap-3 p-4 rounded-lg border border-dashed border-amber-300 dark:border-amber-800 bg-white/60 dark:bg-amber-950/10">
                        <p className="text-slate-700 dark:text-slate-200">
                          ÙÙÙØ² One-Page Plan Ø¨Ø±Ø§Û Ø§ÛÙ Ù¾Ø±ÙÚÙ ÙØ¬ÙØ¯ ÙØ¯Ø§Ø±Ø¯. Ø¨Ø±Ø§Û ÙÙØ§ÛØ´
                          Ø§ÛÙ Ø¨Ø®Ø´Ø ØªÙÙÛØ¯ ÙØ¬Ø¯Ø¯ Ø±Ø§ Ø§Ø¬Ø±Ø§ Ú©ÙÛØ¯.
                        </p>
                        <div className="flex gap-3">
                          <Button
                            className="gap-2"
