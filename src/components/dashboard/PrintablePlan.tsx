...
export const PrintablePlan = forwardRef<HTMLDivElement, { data: BusinessData }>(({ data }, ref) => {
  return (
    <div ref={ref} className="w-full bg-white p-12 text-slate-900" dir="rtl" style={{ direction: 'rtl' }}>
       ...
       <div className="max-w-4xl mx-auto space-y-12">
            ...
            <section>
                <h2 className="text-2xl font-bold border-b-2 border-slate-200 pb-2 mb-4">خلاصه مدیریتی</h2>
                        ...
                        <div className="flex gap-4">
                            {data.colorPalette.map((color, idx) => (
                                <div key={idx} className="flex flex-col items-center gap-2">
                                    ...
                                    />
                                    <span className="font-mono text-sm" dir="ltr">{color}</span>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <section>
                <h2 className="text-2xl font-bold border-b-2 border-slate-200 pb-2 mb-4">برنامه بازاریابی</h2>
            ...
            {data.roadmap && (
                <section className="break-inside-avoid">
                    <h2 className="text-2xl font-bold border-b-2 border-slate-200 pb-2 mb-4">نقشه راه اجرایی (۳۰ روزه)</h2>
                    <div className="grid grid-cols-1 gap-6">
                        {data.roadmap.map((week, idx) => (
                            <div key={idx} className="border border-slate-200 rounded-lg p-4 bg-white">
                                <h3 className="font-bold text-lg mb-2 text-blue-800">{week.week}: {week.focus}</h3>
