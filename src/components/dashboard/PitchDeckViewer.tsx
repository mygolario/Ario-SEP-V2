...
import { ArrowRight, ArrowLeft, AlertTriangle, CheckCircle, TrendingUp, DollarSign } from 'lucide-react';
...
export function PitchDeckViewer({ data }: PitchDeckViewerProps) {
  ...
  const slides = [
    { type: 'title', title: data.businessName, tagline: data.tagline, logo: data.logoSVG },
    ...
    { type: 'business', title: 'مدل کسب‌وکار (Business Model)', revenue: data.leanCanvas?.revenueStream, cost: data.leanCanvas?.costStructure },
  ];
      ...
      <div className="relative aspect-video w-full bg-slate-900 rounded-xl overflow-hidden shadow-2xl border border-slate-700">
        <AnimatePresence initial={false} custom={direction}>
          ...
          >
             ...
             {currentSlideData.type === 'problem' && <SimpleSlide data={currentSlideData as any} theme="red" icon={<AlertTriangle className="w-16 h-16 mb-4 text-red-500" />} />}
             {currentSlideData.type === 'solution' && <SimpleSlide data={currentSlideData as any} theme="green" icon={<CheckCircle className="w-16 h-16 mb-4 text-green-500" />} />}
             {currentSlideData.type === 'market' && <MarketSlide data={currentSlideData as any} />}
             {currentSlideData.type === 'business' && <BusinessSlide data={currentSlideData as any} />}
             ...
             {data.logoSVG && (
                <div className="absolute bottom-6 right-8 w-8 h-8 opacity-50 z-20" dangerouslySetInnerHTML={{ __html: data.logoSVG }} />
             )}
          </motion.div>
        </AnimatePresence>
      </div>
...
function TitleSlide({ data }: { data: { logo: string; title: string; tagline: string } }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center p-12 bg-gradient-to-br from-slate-900 to-slate-800 text-white">
      <div className="w-32 h-32 mb-8 drop-shadow-2xl" dangerouslySetInnerHTML={{ __html: data.logo }} />
...
function SimpleSlide({ data, theme = 'blue', icon }: { data: { title: string; content: string }, theme?: string, icon?: React.ReactNode }) {
    ...
    return (
        <div className="flex-1 flex flex-col items-center justify-center text-center p-16 bg-slate-900 text-white relative overflow-hidden">
             ...
             <div className={`absolute top-0 left-0 w-full h-2 bg-gradient-to-r ${isRed ? 'from-red-600 via-orange-500 to-red-600' : 'from-green-600 via-emerald-500 to-green-600'}`}></div>
             
             {icon}
             <h2 className="text-4xl font-bold mb-8">{data.title}</h2>
             <p className="text-xl text-slate-300 leading-9 max-w-3xl whitespace-pre-wrap">{data.content}</p>
        </div>
    )
}

function MarketSlide({ data }: { data: { title: string; content: { tam: string; sam: string; som: string } } }) {
    const market = data.content;
    return (
        <div className="flex-1 flex flex-col justify-center p-16 bg-slate-900 text-white">
            <h2 className="text-3xl font-bold mb-12 text-center border-b border-slate-800 pb-4">{data.title}</h2>
            ...
            <div className="grid grid-cols-3 gap-8">
             ...
             <p className="text-center text-slate-500 mt-12 text-sm">ارقام تخمینی بر اساس تحلیل اولیه بازار</p>
        </div>
    )
}

function MarketStat({ label, desc, value, color }: { label: string, desc: string, value: string, color: string }) {
    return (
        <div className="flex flex-col items-center text-center p-6 rounded-2xl bg-slate-800/50 border border-slate-700/50">
           <span className="text-6xl font-black text-slate-700 mb-2 opacity-30 select-none">{label}</span>
