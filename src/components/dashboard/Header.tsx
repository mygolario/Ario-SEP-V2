...
import { Menu, ChevronDown, User as UserIcon, Settings, LogOut, Bell } from 'lucide-react';
...
export function Header({ onMenuClick, user, projects = [] }: HeaderProps) {
        ...
        {paths.map((path, index) => {
          ...
          return (
            <React.Fragment key={path}>
              ...
              <span className={cn(isLast && 'font-medium text-slate-900')}>{name}</span>
            </React.Fragment>
          );
        })}
      </div>
  ...
  return (
    <header
      className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-slate-200 z-30 px-4 md:px-8 flex items-center justify-between"
      dir="rtl"
    >
