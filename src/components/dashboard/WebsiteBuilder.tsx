...
import { Monitor, Smartphone, Download, Star, Layout, Globe } from 'lucide-react';
...
export function WebsiteBuilder({ data }: WebsiteBuilderProps) {
  ...
  const content = data.landingPageCopy || {};
  const { headline, subheadline, cta, features, testimonials, footer } = content;
  const logo = data.logoSVG;
  const brandColors = data.colorPalette || ['#4f46e5', '#818cf8', '#c7d2fe'];
...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
    const Icon = LucideIcons[name] || Star;
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm sticky top-4 z-50">
         ...
         <div className="flex items-center gap-4">
             <Tabs value={viewMode} onValueChange={(v: string) => setViewMode(v as 'desktop' | 'mobile')}>
                <TabsList>
                   <TabsTrigger value="desktop"><Monitor className="w-4 h-4 ml-2" /> دسکتاپ</TabsTrigger>
                   <TabsTrigger value="mobile"><Smartphone className="w-4 h-4 ml-2" /> موبایل</TabsTrigger>
                </TabsList>
             </Tabs>

             <Button onClick={handleDownload} variant="outline" className="border-indigo-200 text-indigo-700 hover:bg-indigo-50">
             ...
             {viewMode === 'desktop' && (
                 <div className="bg-slate-50 border-b border-slate-200 p-3 flex items-center gap-4">
                     ...
                     <div className="flex-1 bg-white border border-slate-200 rounded-md px-3 py-1 text-xs text-slate-400 font-mono text-center flex items-center justify-center gap-2">
                        ...
                        {data.businessName || 'your-startup'}.com
                     </div>
                 </div>
             )}
             ...
             <div className="flex-1 overflow-y-auto font-sans text-right" dir="rtl">
                 ...
                 <nav className="flex justify-between items-center p-6 border-b border-slate-100">
                     <div className="font-bold text-xl flex items-center gap-2">
                         {logo ? (
                            <div className="w-8 h-8" dangerouslySetInnerHTML={{ __html: logo }} />
                         ) : (
                     ...
                     <Button size="sm" style={{ backgroundColor: primaryColor }}>ثبت نام</Button>
                 </nav>

                 {/* Hero Section */}
                 <header className="py-20 px-6 text-center bg-gradient-to-b from-slate-50 to-white">
                     <Badge className="mb-6 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border-indigo-100 px-4 py-1">نسخه جدید منتشر شد</Badge>
                     ...
                     <div className="flex flex-col md:flex-row justify-center gap-4">
                        <Button size="lg" className="h-14 px-8 text-lg" style={{ backgroundColor: primaryColor }}>
                           {cta || 'شروع کنید'}
                        </Button>
                 ...
                 <section className="py-24 px-6 bg-white">
                     <div className="max-w-6xl mx-auto">
                         <div className="text-center mb-16">
                            <h2 className="text-3xl font-bold text-slate-900 mb-4">چرا {data.businessName}؟</h2>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                             ...
                             {features?.length > 0 ? features.map((f: any, i: number) => (
                                 <div key={i} className="flex flex-col items-center text-center p-6 rounded-2xl bg-slate-50/50 hover:bg-slate-50 transition-colors">
                                     <div className="w-16 h-16 rounded-2xl bg-white shadow-sm flex items-center justify-center mb-6">
                                         <DynamicIcon name={f.iconName} />
                                     </div>
                                     <h3 className="text-xl font-bold mb-3 text-slate-800">{f.title}</h3>
                                     <p className="text-slate-500 leading-7">{f.description}</p>
                                 </div>
                             )) : (
                                 [1, 2, 3].map((_, i) => (
                     ...
                     <div className="max-w-4xl mx-auto text-center">
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                             ...
                             {testimonials?.length > 0 ? testimonials.map((t: any, i: number) => (
                                 <div key={i} className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl border border-white/10 text-right">
